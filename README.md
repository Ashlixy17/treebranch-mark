...
- 支持 Source 层 Memory Snapshot Cache，减少重复 GitHub API 请求
- 支持使用贡献者 GitHub 头像渲染 Commit 节点，无头像时回退为普通圆点
- 支持英语、简体中文、日语语言切换
- 支持浅色 / 暗夜模式切换
...
`v0.1.0-alpha` 已加入可选 GitHub Token 支持。Token 只属于 GitHub HTTP client 边界，不会进入 `GitSourceInput`、`GitSourceSnapshot`、`RenderPipeline.render(input)`、Parser、Graph Builder、Layout、RenderModel 或 Renderer。
...
- 将 `GitCommit.author.avatarUrl` 映射为显式的 `RenderNode.avatarUrl`
...
- 有头像时将 commit 节点渲染为 32px 圆形 `<image>`
- 无头像时回退为 `<circle>`
- 为 commit 节点渲染短 SHA `<text>`
- 使用 SVG `clipPath` 完成圆形裁剪并安全转义头像 URL
...
```text
docs/releases/v0.1.0-alpha-checklist.md
```

这份 checklist 会作为 `v0.1.0-alpha` 的发布闸门。截图、Token、安全测试、缓存、README、Release Notes、tag 等项目都完成后，再创建正式 release。
...
`v0.1.0-alpha` 已实现 Source 层 Memory Snapshot Cache。

Cache 属于 Source 的数据获取基础设施，不属于 Pipeline。
...
- Pipeline 不知道 Cache 的存在
- Parser / Graph Builder / Layout / RenderModel / Renderer 不知道 Cache 的存在
- 第一版只缓存 `GitSourceSnapshot`
- 第一版只做 Memory Cache
- HTTP ETag / `If-None-Match` / `304 Not Modified` 延后
- Token 不参与 Cache Key

### ADR

项目从 `v0.1.0-alpha` 开始建立 Architecture Decision Records：
...
- 浅色 / 暗夜模式切换
- 可选 GitHub Token 输入
- GitHub API 状态显示：匿名 / 已认证 / Remaining
...
- RenderModel commit author 头像映射
- RenderModel 无头像时显式输出 `null`
...
- SVG Renderer 32px 圆形头像输出
- SVG Renderer 头像 URL XML 转义
- SVG Renderer 无头像 circle fallback
...
- GitHub Token Authorization header
- GitHub Token localStorage 固定 key、自动恢复和清空删除
- GitHub Token 不进入 `GitSourceInput`
- GitHub Token 不进入 Snapshot / Pipeline Result 序列化输出
- GitHub `401` 到 `bad-credentials` 映射
- GitHub `403` 到 `rate-limited` 映射
- GitHub API rate limit headers 状态读取
- Browser GitHub API 状态显示
- Memory Snapshot Cache hit / expire / key isolation
- Memory Snapshot Cache 不缓存失败请求
