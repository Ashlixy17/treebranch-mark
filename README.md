...
- Step 5 前置：RenderModel 架构设计
- 将 `LayoutResult` 转换为 SVG / Canvas / WebGL 可共享的渲染数据模型
...
Step 5  RenderModel       next
Step 6  Renderer          pending
Step 7  Theme / Animation pending
Step 8  Plugin / Action   pending
```
...
- Renderer 永远不应直接依赖布局算法内部细节，后续通过 RenderModel 消费布局结果。
- 每个阶段先明确接口和职责，再进入实现。
